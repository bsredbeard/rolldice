import { compile, pickRandom, randomInt } from 'mathjs';
...
const defaultRoll = (numFaces) => {
  return (options) => {
    const rollResult = randomInt(numFaces)+1;
...
const fudgeRoll = () => {
  return (options) => {
    const roll = pickRandom(ROLL_RANGE);
...
class DiceExpression {
  ...
  execute(){
    if(this.isValid){
      ...
      try{
        const func = compile(this.expression);
